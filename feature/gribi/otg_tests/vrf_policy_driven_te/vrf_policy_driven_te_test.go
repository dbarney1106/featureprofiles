			...
			t.Run("Create ip-in-ip and ipv6-in-ip flows, send traffic and verify decap functionality",
				func(t *testing.T) {
					...
					captureAndValidatePackets(t, args, &packetValidation{portName: portList[0],
						outDstIP: []string{ipv4OuterDst111}, inHdrIP: ipv4InnerDst, inHdrDscp: dscpEncapNoMatch,
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			t.Run("Create ip-in-ip and ipv6-in-ip flows, send traffic and verify decap functionality",
				func(t *testing.T) {
					...
					verifyTraffic(t, args, []string{flow4in4, flow6in4}, !wantLoss)
					captureAndValidatePackets(t, args, &packetValidation{portName: portList[0],
						outDstIP: []string{ipv4OuterDst111}, inHdrIP: ipv4InnerDstNoEncap,
						inHdrDscp: dscpEncapA1, validateTTL: true, validateDecap: true})
				})
		})
	}
}
...
func testGribiDecapMixedLenPref(ctx context.Context, t *testing.T, dut *ondatra.DUTDevice, args *testArgs) {
	...
	t.Run("Verify packets are decap & forward with Default vrf", func(t *testing.T) {
		...
		captureAndValidatePackets(t, args, &packetValidation{portName: portList[0],
			outDstIP: []string{traffiDstIP1}, inHdrIP: ipv4InnerDst,
			inHdrDscp: dscpEncapNoMatch, validateTTL: false, validateDecap: true})
	})
